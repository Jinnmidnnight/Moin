...
from .models import Open_8, Write_8, Comment_8
from .forms import WriteForm_8, Commentform_8
...
def index_8(request):
    open_index = Open_8.objects.all().order_by('-created_at')
    return render(request,'index_8.html',{'open_index' : open_index})

def index_8_1(request):
    open_index = Open_8.objects.all().order_by('-like_count','-created_at')
    return render(request,'index_8_1.html',{'open_index' : open_index})
...
def detail_8(request, open_id):
    ...
    if user_id:
        ...
        return render(request, 'detail_8.html', {'user': user})
    else:
...
def follow_8(request, open_id):
    open = get_object_or_404(Open_8, id=open_id)
...
def write_like_8(request, write_id):
    like_b = get_object_or_404(Write_8, id=write_id)
...
def update_8(request, open_id):
    if request.method == "GET":
        open = get_object_or_404(Open_8, pk = open_id)
        return render(request, "update_8.html", {"open": open})
...
def delete_alert_8(request, open_id):
    open = get_object_or_404(Open_8, pk = open_id)
    return render(request, "delete_alert_8.html", {"open": open})

def delete_8(request, open_id):
    open = get_object_or_404(Open_8, pk = open_id)
    open.delete()
    return redirect('index_8')

@csrf_exempt
def write_update_8(request, open_id, write_id):
    open = get_object_or_404(Open_8, pk=open_id)
    my_write = get_object_or_404(Write_8, pk=write_id)

    if request.method == "POST":
        form = WriteForm_8(request.POST, request.FILES, instance=my_write)
        if form.is_valid():
            write = form.save(commit = False)
            write.author = request.user
            write.post = open
        ...
        return render(request, 'write_update_8.html', {'my_write': my_write})

def write_delete_8(request, write_id):
    write = get_object_or_404(Write_8, pk=write_id)
    post = get_object_or_404(Open_8, pk = write.post.id)
    
    write.delete()
   
    return redirect('detail_8', post.id)

def comment_delete_8(request, comment_id):
    comment = get_object_or_404(Comment_8, pk=comment_id)
    post = get_object_or_404(Write_8, pk = comment.post.id)
    post.comment_count -= 1
    post.save()

    comment.delete()
    return redirect('comment_8', post.id)

def deport_alert_8(request, open_id):
    open = get_object_or_404(Open_8, pk = open_id)
    user_id = request.session.get('user')
    if user_id:
        user = User.objects.get(pk = user_id)
        return render(request, 'deport_alert_8.html', {'user': user})
    else:
        return render(request, 'deport_alert_8.html', {'open' : open})

def deport_8(request, open_id):
    open = get_object_or_404(Open_8, id=open_id)
