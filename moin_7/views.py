...
from .models import Open_7, Write_7, Comment_7
from .forms import WriteForm_7, Commentform_7
...
def index_7(request):
    open_index = Open_7.objects.all().order_by('-created_at')
    return render(request,'index_7.html',{'open_index' : open_index})

def index_7_1(request):
    open_index = Open_7.objects.all().order_by('-like_count','-created_at')
    return render(request,'index_7_1.html',{'open_index' : open_index})
...
def detail_7(request, open_id):
    ...
    if user_id:
        ...
        return render(request, 'detail_7.html', {'user': user})
    else:
...
def follow_7(request, open_id):
    open = get_object_or_404(Open_7, id=open_id)
...
def write_like_7(request, write_id):
    like_b = get_object_or_404(Write_7, id=write_id)
...
def update_7(request, open_id):
    if request.method == "GET":
        open = get_object_or_404(Open_7, pk = open_id)
        return render(request, "update_7.html", {"open": open})
...
def delete_alert_7(request, open_id):
    open = get_object_or_404(Open_7, pk = open_id)
    return render(request, "delete_alert_7.html", {"open": open})

def delete_7(request, open_id):
    open = get_object_or_404(Open_7, pk = open_id)
    open.delete()
    return redirect('index_7')

@csrf_exempt
def write_update_7(request, open_id, write_id):
    open = get_object_or_404(Open_7, pk=open_id)
    my_write = get_object_or_404(Write_7, pk=write_id)

    if request.method == "POST":
        form = WriteForm_7(request.POST, request.FILES, instance=my_write)
        if form.is_valid():
            write = form.save(commit = False)
            write.author = request.user
            write.post = open
        ...
        return render(request, 'write_update_7.html', {'my_write': my_write})

def write_delete_7(request, write_id):
    write = get_object_or_404(Write_7, pk=write_id)
    post = get_object_or_404(Open_7, pk = write.post.id)
    
    write.delete()
   
    return redirect('detail_7', post.id)

def comment_delete_7(request, comment_id):
    comment = get_object_or_404(Comment_7, pk=comment_id)
    post = get_object_or_404(Write_7, pk = comment.post.id)
    post.comment_count -= 1
    post.save()

    comment.delete()
    return redirect('comment_7', post.id)

def deport_alert_7(request, open_id):
    open = get_object_or_404(Open_7, pk = open_id)
    user_id = request.session.get('user')
    if user_id:
        user = User.objects.get(pk = user_id)
        return render(request, 'deport_alert_7.html', {'user': user})
    else:
        return render(request, 'deport_alert_7.html', {'open' : open})

def deport_7(request, open_id):
    open = get_object_or_404(Open_7, id=open_id)
